//
//  ThoiSuViewController.swift
//  News
//
//  Created by hocluan on 11/10/16.
//  Copyright © 2016 hocluan. All rights reserved.
//

import UIKit

class ThoiSuViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    
    // MARK: Outlets
    
    @IBOutlet weak var menuButton: UIBarButtonItem!
    @IBOutlet weak var thoisuTableView: UITableView!
    ...
    override func viewDidLoad() {
        ...
        thoisuTableView.delegate = self
        thoisuTableView.dataSource = self
        thoisuTableView.register(UINib(nibName: "DefaultNewsTableViewCell", bundle: nil), forCellReuseIdentifier: "idDefaultNewsTableViewCell")
        thoisuTableView.rowHeight = UITableViewAutomaticDimension
        thoisuTableView.estimatedRowHeight = 90
        
        // Get data from API URL
        CATEGORY_ID = 1001005
        apiURL = URL(string:"http://api3.vnexpress.net/api/article?type=get_rule_2&cate_id=\(CATEGORY_ID!)&limit=30&app_id=c31b32")
        ...
        URLSession.shared.dataTask(with: apiURL!) { (data, response, error) in
            DispatchQueue.main.async {
                ...
                let data2 = data1?["\(self.CATEGORY_ID!)"]
                self.news = (data2 as! [AnyObject]?)!
                
                self.thoisuTableView.reloadData()
            }
        }.resume()
    }
